class PurchaseHistoriesController < ApplicationController
  ...
  def purchase_history_params
    params.require(:purchase_sending).permit(:post_code, :prefecture_code_id, :city, :house_number, :building_name, :phone_number, :user_id, :item_id).merge(user_id: current_user.id,item_id: params[:item_id],token: params[:token])
  end
